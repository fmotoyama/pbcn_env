...
class PBCN:
    ...
    def controller_to_func(self, controller: np.ndarray):
        ...
        for x_idx in range(2**self.N):
            u_idxs = np.where(self.idx2state(controller[x_idx], self.M))[0]   # controller_funcに項を追加するuのindex
            if len(u_idxs) != 0:
                # xを積で表す項をつくり、controller_funcに追加
                func = ' and '.join(
                    f'x[{i}]' if val==True else f'not x[{i}]'
                    for i,val in enumerate(self.idx2state(x_idx, self.N))
                    )
                for u_idx in u_idxs:
    ...
    def controller_to_func_minimum(self, controller):
        ...
        for x_idx in range(2**self.N):
            ...
            if len(u_idxs) != 0:
                # xを積で表す項をつくり、controller_funcに追加
                func = [(i+1)*[-1,1][int(val)] for i,val in enumerate(self.idx2state(x_idx, self.N))]
                for u_idx in u_idxs:
...
if __name__ == '__main__':
    ...
    a = env.state2idx(np.array([True,False,False]))
